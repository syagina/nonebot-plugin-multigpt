...
async def handle_chat_record(bot: Bot, event: MessageEvent, msg: Message = CommandArg()):
    ...
    if manager.get_entry(session_id) is None:
        ...
        await chat_record.send(MessageSegment.text("本机器人为ai工具机器人，详细功能与指令见https://github.com/syagina/nonebot-plugin-multigpt\n无法访问请关闭WiFi使用流量"))
    ...
    try:
        ...
        if content.startswith("画："):
            ...
            await chat_record.send(MessageSegment.text("如果要画画请回复："))
            await chat_record.send(MessageSegment.text("画："+content))
        try:           
        ...
        if response=="error":
            await chat_record.finish(MessageSegment.text("与openai请求发生错误，请稍后再试"))
