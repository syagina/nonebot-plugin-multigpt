...
from nonebot.adapters.onebot.v11 import (
    ...
    MessageSegment,
    MessageEvent,
    helpers,Bot, FriendRequestEvent
)
...
async def _(bot: Bot, event: MessageEvent, state: T_State, content: str = ArgPlainText()):
    ...
    aa=f'D:\\gpt\\data\\nonebot-plugin-multigpt\\theme\\{theme}\\{template}'
    state["bb"]=aa.replace('.pptx', '.png')
    
    png_filepath = filepath.replace('.pptx', '.png')
    ...
    if not os.path.exists(png_filepath):
        ...
        await start_request.send(MessageSegment.image(file=state["bb"],cache=True))
...
async def _(bot: Bot, event: MessageEvent, state: T_State, confirm: str = ArgPlainText()):
    ...
    try:
        ...
        if "layout index out of range" in str(error):
            ...
            delete_file(os.path.join(cache_folder))
